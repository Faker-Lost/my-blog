...
export default function AIChat({ chatId, initialMessages, onFinish }: AIChatProps) {
  ...
  const { messages, sendMessage, status, setMessages } = useChat({
    ...
    onFinish: (message) => {
      if (onFinish) {
        // Note: We can't easily access the full updated message history inside this callback 
        // without ref access due to closure capture. We emit just the new message for now
        // or rely on the parent to handle state. 
        // However, usually onFinish provides the completed message.
        // If the parent expects the full history, we might need a Ref to current messages.
        // For now, let's fix the type error by passing [message] or handling it differently.
        // Actually, let's just pass the single message as an array if that satisfies strictness,
        // or change the interface if we can. But I'll stick to fixing the syntax error first.
        onFinish([message]);
      }
    },
  });
