...
export default function AIChat({ chatId, initialMessages, onFinish }: AIChatProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-4xl mx-auto border-x border-zinc-200 bg-white/90 backdrop-blur-xl shadow-xl">
      {/* Messages Area */}
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-10 scroll-smooth scrollbar-thin scrollbar-thumb-zinc-300 scrollbar-track-transparent"
      >
        {messages.length === 0 && (
          <motion.div
          ...
          >
            <div className="p-5 rounded-3xl bg-cyan-50 border border-cyan-100 shadow-sm">
              <Bot className="w-14 h-14 text-cyan-600" />
            </div>
            <div className="space-y-2">
              <h1 className="text-3xl font-bold tracking-tight text-zinc-900 font-mono italic">
                FAKER :: AI STUDIO
              </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg pt-8">
              {['分析 C++ 内存对齐', '解释 OpenCV 特征提取', 'Code is Reality?', '优化 Qt 异步任务'].map(q => (
                <button
                  ...
                  className="group relative p-5 text-left border border-zinc-200 rounded-2xl bg-white hover:bg-white hover:border-cyan-500/50 transition-all overflow-hidden shadow-sm hover:shadow-md active:scale-[0.98]"
                >
                  ...
                  <span className="text-[10px] font-mono text-zinc-400 group-hover:text-cyan-600 uppercase tracking-widest mb-1.5 block font-bold">Prompt</span>
                  <span className="text-sm text-zinc-700 group-hover:text-zinc-900 font-semibold transition-colors">{q}</span>
                </button>
              ))}
            </div>
          </motion.div>
        ...
        <AnimatePresence mode="popLayout">
          {messages.map((m: any) => (
            ...
            >
              {/* Avatar */}
              <div className={clsx(
                "w-9 h-9 sm:w-10 sm:h-10 rounded-xl shrink-0 flex items-center justify-center border shadow-sm",
                m.role === 'user'
                  ? "bg-zinc-100 border-zinc-200"
                  : "bg-cyan-50 border-cyan-100"
              )}>
                {m.role === 'user' ? <User className="w-5 h-5 text-zinc-500" /> : <Terminal className="w-5 h-5 text-cyan-600" />}
              </div>
