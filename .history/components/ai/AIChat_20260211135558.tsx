...
export default function AIChat({ chatId, initialMessages, onFinish }: AIChatProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-4xl mx-auto border-x border-zinc-200 dark:border-zinc-800 bg-white/90 dark:bg-zinc-900/95 backdrop-blur-xl shadow-xl">
      ...
      >
        ...
        <AnimatePresence mode="popLayout">
          {messages.map((m: any) => (
            ...
            >
              ...
              {/* Message Content */}
              <div className={clsx(
                "flex-1 max-w-[85%] rounded-2xl p-5 sm:p-6 text-base sm:text-[17px] leading-relaxed shadow-sm dark:shadow-md",
                m.role === 'user'
                  ? "bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-50 rounded-tr-none border border-zinc-200 dark:border-zinc-700 dark:shadow-zinc-950/20"
                  : "bg-white dark:bg-zinc-800/40 text-zinc-800 dark:text-zinc-100 rounded-tl-none border border-zinc-100 dark:border-zinc-700/50 shadow-inner-sm"
              )}>
                {/* Reasoning Part */}
                {m.parts?.some((p: any) => p.type === 'reasoning') && (
                  <div className="mb-6 p-4 rounded-xl bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-100 dark:border-zinc-700/30 text-sm text-zinc-500 dark:text-zinc-400 font-mono leading-relaxed italic dark:shadow-inner">
                    <div className="flex items-center gap-2 mb-3 text-cyan-600/70 dark:text-cyan-500/70 uppercase tracking-widest font-black text-[10px]">
                ...
                <div className="prose prose-zinc dark:prose-invert max-w-none break-words font-medium">
                  <ReactMarkdown
                    ...
                    components={{
                      code({ node, inline, className, children, ...props }: any) {
                        return !inline ? (
                          ...
                          <code className="bg-zinc-100 dark:bg-zinc-950/50 px-2 py-0.5 rounded text-cyan-700 dark:text-cyan-300 text-[13px] font-mono border border-zinc-200/50 dark:border-zinc-700/50 font-bold" {...props}>{children}</code>
                        )
                      }
                    }}
                  >
        ...
        {(status === 'submitted' || status === 'streaming') && (
          <div className="flex gap-4 sm:gap-6 animate-pulse">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-cyan-50 dark:bg-cyan-950/20 flex items-center justify-center border border-cyan-100 dark:border-cyan-900/30">
              <Sparkles className="w-5 h-5 text-cyan-600 dark:text-cyan-500/50" />
            </div>
            <div className="space-y-2 flex-1 pt-2">
              <div className="h-4 bg-zinc-100 dark:bg-zinc-800 rounded w-24" />
              <div className="h-4 bg-zinc-100/50 dark:bg-zinc-800/50 rounded w-full max-w-md" />
            </div>
          </div>
        )}
      </div>
      ...
      {/* Input Area */}
      <div className="p-4 sm:p-6 border-t border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/90 backdrop-blur-md">
        <form
        ...
        >
          <input
            ...
            className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-2xl py-5 pl-7 pr-16 text-base text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-cyan-500 dark:focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/10 dark:focus:ring-cyan-500/20 transition-all placeholder:text-zinc-400 dark:placeholder:text-zinc-500 font-medium dark:shadow-inner"
          />
          <button
            type="submit"
            disabled={status !== 'ready' || !inputValue.trim()}
            className="absolute right-2 top-2 bottom-2 px-5 rounded-xl bg-cyan-600 hover:bg-cyan-500 disabled:bg-zinc-100 dark:disabled:bg-zinc-800 disabled:text-zinc-400 dark:disabled:text-zinc-600 text-white transition-all shadow-md hover:shadow-cyan-100 dark:hover:shadow-cyan-500/20 disabled:shadow-none flex items-center justify-center"
          >
            <Send className="w-4 h-4" />
          </button>
        </form>
        <div className="mt-4 flex justify-center items-center gap-4 text-[11px] text-zinc-500 dark:text-zinc-600 font-mono uppercase tracking-[0.25em] font-bold">
          <span>Algorithm is Power</span>
          <span className="w-1 h-1 rounded-full bg-zinc-300 dark:bg-zinc-700 rotate-45" />
