...
export async function POST(req: Request) {
  ...
  for (const key in MOCK_RESPONSES) {
    if (userText.toLowerCase().includes(key.toLowerCase())) { // Ensure case-insensitive matching
      ...
      const interval = setInterval(() => {
        if (index < responseText.length) {
          ...
          dataStream.writeChunk({
            type: 'text-part',
            text: responseText.slice(index, index + 5)
          });
          index += 5;
        } else {
          clearInterval(interval);
          dataStream.writeChunk({ type: 'finish-step', finishReason: 'stop', usage: { completionTokens: 0, promptTokens: 0 } });
        }
      }, 30);
    },
  });
